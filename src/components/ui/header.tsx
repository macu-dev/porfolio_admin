...
const Header = ({ logo, links, homeLink = '/' }: HeaderProps) => {
  ...
  return (
    <header
      className={`sticky top-0 z-10 bg-transparent backdrop-blur-sm transition duration-200 ease-in-out ${hasScrolled ? 'border-b border-stone-800  bg-[#0e0e0ee6]' : 'border-transparent'}`}
    >
          ...
          >
            <ul className="mt-4 flex flex-col font-medium lg:mt-0 lg:flex-row lg:space-x-8">
              {links?.map((link) => (
                <li key={link.name}>
                  <Link
                    className="lg:hover:text-primary-700 block whitespace-nowrap border-b border-gray-100 py-2 pl-3 pr-4 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white lg:border-0 lg:p-0 lg:hover:bg-transparent lg:dark:hover:bg-transparent lg:dark:hover:text-white"
                    href={link.url}
                  >
                    {link.name}
                  </Link>
